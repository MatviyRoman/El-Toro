...
if (
	process.env.NODE_ENV.trim() === 'imagemin' ||
	process.env.NODE_ENV.trim() === 'imgmin'
) {
	imagemin = 2;
}

// mix.minTemplate = require('laravel-mix-template-minifier').default; //npm install --save-dev laravel-mix-template-minifier

// if (mix.inProduction()) {
// 	mix.minTemplate(dev + '/*.php', prod + '/views/');
// }
...
if (imagemin == 1) {
	...
	if (jpgToWebp) {
		...
		compress_images(
			dev + folderImg + '**/*.{jpg,JPG,jpeg,JPEG,gif,png,svg}',
			prod + folderImg,
	...
	if (jpgToJpg) {
		//[jpg(jpegtran)] ---to---> [jpg(mozjpeg)] WARNING!!! autoupdate  - recommended to turn this off, it's not needed here - autoupdate: false //this
		compress_images(
			dev + folderImg + '**/*.{jpg,JPG,jpeg,JPEG}',
			prod + folderImg,
...
mix.js(dev + '/assets/js/main.js', prod + '/assets/js/main.js');

// mix.sass(dev + '/assets/scss/normalize.scss', prod + '/assets/css/');

mix.sass(dev + '/assets/scss/style.scss', prod + '/assets/css/')
	.options({
		postCss: [
...
//copy
// mix.copy(dev + '/template-parts/**/**/*.php', prod + '/template-parts/');
// mix.copy(dev + '/inc/**/**/*.php', prod + '/inc/');
// mix.copy(dev + '/scss/**/**/*.scss', prod + '/css/');
// mix.copy(dev + '/images/**/**/*.*', prod + '/images/');
// mix.copy(dev + '/video/**/**/*.*', prod + '/video/');
// mix.copy(dev + '/gigpress-templates/**/**/*.*', prod + '/gigpress-templates/');

mix.copy(dev + '/assets/fonts/**/**/*.*', prod + '/assets/fonts/');
mix.copy(dev + '/index.html', prod + '/index.html');

mix.copy(dev + '/assets/js/**/**/*.*', prod + '/assets/js/');
mix.copy(dev + '/assets/*.*', prod + '/');

// mix.browserSync({
// 	proxy: 'http://ryanvet.apartner.top', // your domain test
// 	port: 3000, // your port
// });
...
if (
	...
	//min files
	mix.minify(prod + '/assets/js/main.js');

	mix.styles(
		[
			// other css stylesheets here...
			prod + '/assets/css/style.css', // include temp css file
			//prod + '/assets/css/app.css', // include temp css file
		],
		prod + '/assets/css/style.min.css'
	).then(() => {
		// del(prod + '/css/style.css'); // deletes the temp file
	});
	...
	mix.sourceMaps(prod + 'assets/css/style.css');
}
